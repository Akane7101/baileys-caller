...
export class WasmEngine {
    ...
    acceptCall = (isMicEnabled = true, isCameraEnabled = false) => {
        ...
        let hasAccept = false;
        try {
            hasAccept = typeof this.#instance.acceptCall === "function";
        }
        catch { }
        if (!hasAccept) {
            throw new Error(`WASM exposes no acceptCall; call-related methods present: ${this.callMethodNames().join(", ") || "none"}`);
        }
    ...
    #startAudioPlaybackLoop = () => {
        ...
        this.#audioPlaybackLoopInterval = setInterval(() => {
            ...
            try {
                ...
                if (hasNonZero)
                    this.#config.callbacks?.onAudioPlaybackData?.(view.slice());
            }
            catch { }
        }, 16);
    };
