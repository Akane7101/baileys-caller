...
export class SignalingBridge {
    ...
    #doSendSignaling = async (peerJid, callId, xmlPayload) => {
        ...
        if (isGroupOffer) {
            const kids = getNodeChildren(voipNode).map((c) => c.tag).join(",");
    ...
    #doProcessIncomingCall = async (node, voip, activeCallId) => {
        ...
        if (usableNode.tag === "offer") {
            ...
            const childTags = (getAllBinaryNodeChildren(usableNode) || []).map((c) => c.tag);
            console.log(`[baileys-caller] inbound offer ${incomingCallId} shape: attrs=[${attrKeys.join(",")}]` +
                ` children=[${childTags.join(",")}] caller_pn=${usableNode.attrs?.caller_pn ?? "-"}`);
        }
    ...
    #maybeDecryptEnc = async (voipNode, peerJid) => {
        ...
        for (const jid of candidates) {
            try {
                ...
                lastErr = err;
                attempts.push(`${jid}: ${err?.message || err}`);
            }
        }
